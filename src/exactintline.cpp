...
int main(int argc, char* argv[ ]) {
    setName("compare lines as sequences of arbitrary-length signed integers");
    ...
    while (!ans.eof()) {
        ...
        if (jnl && pnl) {
            ...
            quitf(
                _wa, "Extra %d%s number in %d%s line: '%s'",
                m, englishEnding(m).c_str(), n, englishEnding(n).c_str(), compress(p).c_str()
            );
        else if (pnl && !jnl)
            quitf(
                _wa, "Missing %d%s number in %d%s line: '%s' expected",
